AutoReg <- function(){
  ...
  # STEP 1.5: Capture response variable
  repeat{
    st.row <<- as.character(readline("Please enter the row number you want to start modeling: "))
    cat("\n")
    if(all(strsplit(st.row, split = "")[[1]] %in% as.character(0:9))) {
      st.row <<- as.numeric(st.row)
      break
    }else{
      ...
      if (toupper(rec) == "N"){
        ...
        repeat{
          ...
          if (endstr != "csv"){
            ...
            rebuild(resp, data = df0, prmt.name, st.row)
            prmt <<- prmt.history
            #prmt.alive <<- prmt[which(prmt[[8]] == "alive"), ]
            #return(step2.4())
            break
          }else{
  ...
  repeat{
    ...
    if(toupper(incpt) == "Y"){
      if(is.null(fit.history)){
        df.temp0 <<- df0
        df1 <<- df.temp <<- df.temp0[st.row:dim(df.temp0)[1], ]
        fit1 <<- fit.temp <<- trial(data = df.temp, resp = resp, fit = NULL, action = 1)
        
      }else{
        df.temp0 <<- df.history0
        df1 <<- df.temp <<- df.history
        fit1 <<- fit.temp <<- trial(data = df.temp, resp = resp, fit = fit.history, action = 1)
      }
      ...
      if(is.null(fit.history)){
        df.temp0 <<- df0
        df.temp <<- df.temp0[st.row:dim(df.temp0)[1], ]
        fit.temp <<- trial(data = df.temp, resp = resp, fit = NULL, action = -1)
      }else{
        df.temp0 <<- df.history0
        df1 <<- df.temp <<- df.history
        fit1 <<- fit.temp <<- trial(data = df.history, resp = resp, fit = fit.history, action = -1)
      }
      break
    }else{
  ...
  if(!is.null(prmt.history)){
    ...
    warn(fit1 = NULL, fit2 = fit.temp, p.cons = 0.2)
    readline("Continue modeling... \nPress <Enter> to continue...\n")
    
    repeat{
      rev.his <<- toupper(readline("Do you want to remove a predictor / an intercept from the model (Y/N)? "))
      cat("\n")
      if(!rev.his %in% c("Y", "N")){
        message('Only "Y" or "N" is acceptable!\n')
      } else {
  ...
  #----------------------------------------------------
  # STEP 2.1: 
  step2.1 <- function(key1 = 0, key2 = 0){
    # get the name of predictor and check its existence in the database and in the model
    if(key1 == 0){
      repeat{
        pred.i <<- pred <<- pred.i <- pred <- as.character(readline("Please enter the name of predictor you want to add: "))
        cat("\n")
        if (!pred %in% names(df.temp)){
          message(paste('The predictor "', pred, '" does not exist in the database!', sep=""))
          cat("\n")
        }else if(pred %in% prmt.alive[[1]]){
          message(paste('The predictor "', pred, '" has been chosen before!', sep=""))
          cat("\n")
          repeat{
            cho <- toupper(readline(paste("Do you want to continue with", pred, "(Y/N)? ", sep = " ")))
            cat("\n")
            if(!cho %in% c("Y", "N")){
              message('Man (or u r a woman/girl...)! Only "Y" or "N"!')
            } else {
              if(cho == "N") {
                return(step2.1())
              } else {
                return(step2.1(key1 = 1))
              }
            }
          }
        }else{
    ...
    step2.2 <- function(key = 0, opt1 = NULL){
      ...
      repeat{
        ...
        if(!opt1 %in% c("1.1","1.2","1.3","0","2.1.1","2.1.2","2.1.3","2.1.4","2.2","2.3","2.4","2.5","2.6")){
          ...
          # object.trans <- 
          #----------------------------------------------------------------------------
          if((pred %in% prmt.alive[, 1])){
            if(opt1 %in% prmt.alive[which(prmt.alive[, 1] == pred), 2]){
              message("You've chosen to add an existed variable in the model with the same trans-type.\n")
              repeat{
                cho <- toupper(readline(paste("Do you want to continue with", pred, "(Y/N)? ", sep = " ")))
                cat("\n")
                if(!cho %in% c("Y", "N")){
                  ...
                  if(cho == "N") {
                    ...
                    return(step2.2(key = 1, opt1 = opt1))
                  }
                }
              }
              ...
              repeat{
                ...
                if(all(strsplit(obj2, split = "")[[1]] %in% c(as.character(0:9), "."))) {
                  if(as.numeric(obj2) == 0){
                    ...
                    cat("\n")
                    chg <- readline("Do you want to change transformation method (Y/N)? ")
                    cat("\n")
      ...
      } # end of the large repeat in Step2.2
      #----------------------------------------------------------------------------
      ...
      prmt.recom <- recom(pred = pred, resp = resp, data = df.temp0, 
                          type = opt.trans, fit = fit.temp, object = object.trans, obj.name = obj.name, st.row)
      
      if(all(prmt.recom == 1)) return(step2.1(key2 = 1))
      ...
      step2.4 <- function(key = 0){
        ...
        #---------------------------------------------------------
        # Definite 2 functions to avoid long replication codes
        ...
        if(opt2 == 1){
          ...
          pred.name <<- NULL
          df.temp0 <<- modif(pred, type = opt.recom, df.temp0, co.r.recom, sc.1.recom, sc.2.recom, pc.r.recom, obj.recom, obj.name)
          df.temp <<- df.temp0[st.row:dim(df.temp0)[1], ]
          ...
          if(length(coef(fit.temp)) != length(coef(fit1))) {
          ...
          } else if(sum(coef(fit.temp) != coef(fit1)) != 0) {
            loop.output(resp, df.temp, fit.temp)
            warn(fit1 = fit1, fit2 = fit.temp, p.cons = 0.2)
            # p.cons = 0.2: p-value cannot excceed 20%, otherwise wanrning message generated
          }
          ...
          if(rev.his == "N"){
            ...
            prmt.temp <- data.frame(pred.i, type = s.prmt[1], co.r = s.prmt[2], 
                                    ...
                                    status = "alive", new.name = if(is.null(pred.name)) pred else pred.name, 
                                    stringsAsFactors = F)
          ...
          if(opt.trans %in% c("1.1","1.2","2.1.1","2.1.2","2.1.3","2.1.4","2.2","2.3","2.4","2.5","2.6")){
            ...
            if(opt == "a"){
              cat(
                paste(" The carry over rate is ", co.r.recom, sep = ""),
                paste(" The lamda1 of S curve is ", sc.1.recom, sep = ""), 
                paste(" The lamda2 of S curve is ", sc.2.recom, sep = ""), sep = "\n")
            } else if(opt == "b"){
              cat(
                paste(" The carry over rate is ", co.r.recom, sep = ""),
                paste(" The power rate is ", pc.r.recom, sep = ""), sep = "\n")
            }
          ...
          # OPTION 2.4.4
          if(!is.null(pred.name)){
            df.temp0 <<- df.temp0[, - which(names(df.temp0) == pred.name)]
          } else {
              df.temp0[, pred] <<- df0[, pred]
          }
          
          df.temp <<- df.temp <- df.temp0[st.row:dim(df.temp0)[1], ]
          fit.temp <<- fit.temp <- fit1
          
          pred.name <<- NULL
          
          return(step2.1(key2 = 1))
        }
    ...
    step2.6 <- function(key = 0, opt3 = NA){
      if(key == 0){
        repeat{
      ...
      if(opt3 == 1){
        ...
        step2.6.2 <- function(){
          ...
          repeat{
            ...
            message("The name of predictor (case sensitive) should be provided below.")
            message('If you want to delete the intercept, please enter "intercept".')
            cat("\n")
            ...
            if(toupper(var.tbr) == "INTERCEPT"){
              if("(Intercept)" %in% names(coef(fit1))){
                ...
                repeat{
                  cat("\n")
                  cont <- toupper(readline("Do you still want to remove a predictor / the intercept (Y/N)? "))
                  cat("\n")
                  if(cont == "N"){
                    return(step2.6())
              ...
              if(!var.tbr %in% names(df.temp)){
                ...
                df.temp <<- df1
                df.temp[[var.tbr]] <<- df0[[var.tbr]][st.row:dim(df0)[1]]
          ...
          repeat{
            ...
            if(toupper(conf.rmv) == "Y"){
              ...
              if(var.tbr %in% names(df0)){
                df.temp0[, var.tbr] <<- df0[, var.tbr]
              } else {
                df.temp0 <<- df.temp0[, - which(names(df.temp0) == var.tbr)]
              }
              ...
              if(toupper(var.tbr) != "INTERCEPT"){
                prmt[which((prmt[[8]] == "alive") & (prmt[[9]] == var.tbr)), 8] <<- "dead"
                prmt.alive <<- prmt[which(prmt[[8]] == "alive"), ]
              }
  ...
  #   * can't realize the case to clear the environment after the whole modeling process ends
  #
  ...
  #
  # MODIFIED IN VERSION 2.2 in Dec. 2014
  #   * can choose a row number of the initial data to start modeling
  #   * initialize var in df.temp after removing, to avoid double or more times of transformation in one single var
  #   
  #   Existed problems on Tuesday 12/08/2014:
  #   1. cannot actually exit from the program: after exporting, loop return to the start of step2.1()
  #   2. cannot add predictor to an imported model: add new predictor after chosen trans-method
  #      Error in st.row:dim(df0)[1] : argument of length 0 
  #   3. sometimes after choose No-Transformation
  #      Error in summary(fit.new)$coefficients[length(coef(fit.new)), 1] : subscript out of bounds
  #
  # MODIFIED IN VERSION 2.2 on Thursday 12/18/2014
  #   * Successfully exit the program: 
  #         by add key2 to step2.1() to control whether execute step 2.5 and step 2.6
  #         Problem is after choosing step 2.4 option 4: change a variable
  #   * Can add new predictor to an imported model
  #
  # MODIFIED IN VERSION 2.3 on Friday 12/19/2014
  #   * Avoid replicate codes in step 2.4 for changing parameter of trans-method 1.1, 1.2, 1.3
  #     By definiting two new small functions opt1.1() and opt1.2()
  #   * Global Environment variable "prmt" doesn't record correctly
