...
loop.output <- function(resp, data, fit) {        				
    ...
    rownames(contri.top10) <- rownames(contri.p)[which(rownames(contri.p) !="(Intercept)")]
    ...
    rownames(contri.bot5) <- rownames(contri.n)[which(rownames(contri.n) != "(Intercept)")]
...
final.output <- function(resp, data, fit, prmt, contri, aic = FALSE) {
  ...
  if(rownames(coef)[1] == "(Intercept)"){
    ...
    prmt.alive$pred <- as.vector(prmt.alive$pred) 
