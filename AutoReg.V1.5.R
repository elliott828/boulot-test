AutoReg <- function(data){
  ...
  # -------------------------------------------- #
  # Asking if a record of model needs to be read #
  # -------------------------------------------- #
  
  repeat{
    cat("\n")
    rec <- readline("Do you want to continue testing a existed model (Y/N)? ")
    cat("\n")
    if (toupper(!toupper(rec) %in% c("Y","N"))){
      message("Please only enter Y or N!")
    }else{
      if (toupper(rec == "Y")){
        contm <- ContM(resp, ndf)
        fit <- contm[[1]]
        ndf <- contm[[2]]
        prmt <- contm[[3]][,2:7]
        prmt[,1] <- as.character(prmt[,1])
        # make the following predictor acceptable by prmt
        # prmt here is read as a data frame and "variable" is a factor
        # need to change the type of "variable" to "char"
        break
      }else{
        break
      }
